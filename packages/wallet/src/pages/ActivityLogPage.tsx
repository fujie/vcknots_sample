          ...
          {logs.map((entry) => (
            <div key={entry.id} className="flex items-center gap-3 p-3 rounded-lg bg-gray-50 border border-gray-100">
              ...
              <div className="flex-1 min-w-0">
                ...
                {entry.errorReason && <p className="text-xs text-red-600 mt-0.5">{entry.errorReason}</p>}
              </div>
              <span className="text-xs text-gray-400 flex-shrink-0">{new Date(entry.timestamp).toLocaleTimeString()}</span>
            </div>
          ))}
        </div>
      )}
